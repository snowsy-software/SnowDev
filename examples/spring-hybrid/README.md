...
The application runs on the host (`mvn spring-boot:run`); Docker Compose only
...
This standalone example includes a minimal Spring Boot application. It requires
JDK 17+ and Maven 3.9+ on the host.
...
```bash
npm run dev        # db up -> health wait -> mvn validate hook -> mvn spring-boot:run (attached)
...
The `afterDependenciesReady` hook calls `exec("mvn", ["-q", "validate"])`
