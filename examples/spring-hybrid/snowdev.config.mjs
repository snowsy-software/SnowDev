...
const host = { command: "mvn", args: ["spring-boot:run"], stopGraceSeconds: 15 };
...
export default {
  ...
  hooks: {
    ...
    async afterDependenciesReady({ exec, log }) {
      log("afterDependenciesReady: validating the Maven project");
      const { exitCode } = await exec("mvn", ["-q", "validate"]);
      if (exitCode !== 0) throw new Error(`mvn validate exited with ${exitCode}`);
    },
  },
};
